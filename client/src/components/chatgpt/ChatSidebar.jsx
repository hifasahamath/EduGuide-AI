    ...
    <div className={`w-[260px] h-full ${bg} flex flex-col border-r ${borderColor} transition-colors duration-300 select-none`}>
      ...
      <div className={`px-4 py-3.5 flex items-center justify-between border-b ${borderColor}`}>
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex flex-col">
            <span className={`font-extrabold text-sm tracking-tight ${textMain}`}>EduGuide AI</span>
      ...
      <div className="px-3 pb-2">
        <div className="relative">
          ...
          <input type="text" placeholder="Search sessions..." value={search}
            ...
            className={`w-full rounded-xl py-2 pl-8 pr-3 text-xs font-semibold border focus:outline-none focus:ring-2 focus:ring-indigo-500/40 transition-all ${inputBg}`}
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-2 py-1 custom-scrollbar space-y-0.5">
        ...
        {loadingChats ? (
              ...
              <motion.div key={chat.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -8 }}>
                {renamingId === chat.id ? (
                  ...
                  <button onClick={() => handleSelectChat(chat.id)}
                    ...
                    <MessageSquare size={14} className={`flex-shrink-0 opacity-80 ${chat.pinned ? 'text-indigo-400' : ''}`} />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1">
                        <span className="truncate block font-semibold">{chat.title || 'New Chat'}</span>
          ...
          <p className={`text-xs text-center py-8 font-medium ${textMuted}`}>
      ...
      <div className={`border-t ${borderColor} p-2 space-y-0.5`}>
        ...
        <button onClick={toggleTheme}
          ...
          <div className="relative flex items-center" style={{ width: 30, height: 16 }}>
            <div className={`w-7 rounded-full transition-colors ${theme === 'dark' ? 'bg-indigo-600' : 'bg-slate-400'}`} style={{ height: 16 }}>
