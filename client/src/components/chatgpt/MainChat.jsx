...
const WhatsAppIcon = ({ size = 16, className = "flex-shrink-0" }) => (
  <svg
    viewBox="0 0 24 24"
    width={size}
    height={size}
      ...
      <div className={`max-w-[95%] sm:max-w-[88%] md:max-w-[85%] min-w-0 ${isBot ? '' : 'flex flex-col items-end'}`}>
        ...
        {isBot && msg.showWhatsApp && (
          <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent('Hello, I need help..')}`}
            ...
            className="mt-2.5 inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-[#25D366] to-[#128C7E] hover:from-[#20bd5a] hover:to-[#0e7065] text-white text-xs font-bold shadow-sm shadow-emerald-600/30 hover:shadow-md hover:shadow-emerald-600/40 border border-emerald-400/30 transition-all duration-200 active:scale-[0.98] cursor-pointer touch-manipulation">
            <WhatsAppIcon size={16} className="text-white" />
            <span>Contact Advisor / Developer</span>
          </a>
        )}
      </div>
    </motion.div>
    ...
    <div className={`flex-1 flex flex-col h-full ${bg} transition-colors duration-300 relative min-w-0`}>
      ...
      <div className={`sticky top-0 z-10 ${headerBg} backdrop-blur-md border-b px-3 sm:px-4 py-2.5 sm:py-3 flex items-center justify-between gap-2`}>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
          ...
          {/* WhatsApp Action Button with authentic WhatsApp branding & Education Advisor / Developer label */}
          <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent('Hello, I need help..')}`}
            target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-2 text-xs px-3 sm:px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-[#25D366] to-[#128C7E] hover:from-[#20bd5a] hover:to-[#0e7065] text-white font-bold shadow-sm shadow-emerald-600/30 hover:shadow-md hover:shadow-emerald-600/40 border border-emerald-400/30 transition-all duration-200 active:scale-[0.98] cursor-pointer touch-manipulation group"
            title="Chat with Education Advisor / Developer on WhatsApp">
            <WhatsAppIcon size={16} className="text-white flex-shrink-0 drop-shadow-xs" />
            <span className="hidden sm:inline">Education Advisor / Developer</span>
            <span className="sm:hidden">Advisor / Developer</span>
          </a>
        </div>
      </div>
