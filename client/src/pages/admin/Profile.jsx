...
const Input = ({ ...props }) => (
  <input {...props} className="w-full border border-gray-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-white dark:bg-[#151525] text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500" />
);

const Toggle = ({ checked, onChange, label, description }) => (
  <div className="flex items-start justify-between gap-4 py-3 border-b border-gray-50 dark:border-white/5 last:border-0">
    <div>
    ...
    <button onClick={() => onChange(!checked)}
      className={`relative inline-flex h-6 w-11 flex-shrink-0 rounded-full border-2 border-transparent transition-colors ${checked ? 'bg-indigo-600' : 'bg-gray-200 dark:bg-white/10'}`}>
...
const Profile = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white dark:bg-[#1a1a2c] rounded-2xl border border-gray-100 dark:border-white/10 shadow-sm overflow-hidden">
        ...
        <div className="px-8 pb-6">
          <div className="flex items-end justify-between -mt-10 mb-5">
            <div className="bg-white dark:bg-[#1a1a2c] p-1.5 rounded-full border-4 border-white dark:border-[#1a1a2c] shadow-lg">
              <div className="w-16 h-16 rounded-full bg-indigo-100 dark:bg-indigo-500/20 flex items-center justify-center">
                <User size={34} className="text-indigo-600 dark:text-indigo-400" />
              </div>
            </div>
      ...
      {/* Tabs */}
      <div className="flex gap-1 p-1 bg-gray-100 dark:bg-white/5 rounded-2xl overflow-x-auto">
        {TABS.map(t => (
